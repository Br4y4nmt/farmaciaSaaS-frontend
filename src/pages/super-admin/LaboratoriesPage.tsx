...
function LaboratoriesPage() {
  ...
  const [editingLaboratory, setEditingLaboratory] = useState<Laboratorio | null>(null)
  const { laboratorios, isLoading, error, refetch } = useLaboratorios(user?.empresa_id)
  ...
  const columns = [
    { key: 'codigo', header: 'Código', render: (lab: Laboratorio) => lab.codigo || '-' },
    { key: 'nombre', header: 'Laboratorio' },
    { key: 'descripcion', header: 'Descripción', render: (lab: Laboratorio) => lab.descripcion || '-' },
    { key: 'total_productos', header: 'Productos', render: (lab: Laboratorio) => lab.total_productos ?? 0 },
    { key: 'estado', header: 'Estado', render: (lab: Laboratorio) => (
      <span className={`rounded-full px-3 py-1 text-xs font-medium ${lab.estado ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
